...
class BombeRLeWorld(GenericWorld):
    def __init__(self, args: WorldArgs, agents, stutter):
        super().__init__(args)

        # Controls how many rounds is the same map reused
        self.stutter = stutter
        self.setup_agents(agents)
        self.last_arena = None
        self.last_coins = None
        self.last_start_position_permutation = None
    ...
    def build_arena(self):
        if self.round % self.stutter == 0:
            WALL = -1
            FREE = 0
            CRATE = 1
            self.last_arena = np.zeros((s.COLS, s.ROWS), int)
            ...
            for (x, y) in start_positions:
                for (xx, yy) in [
                    ...
                    if self.last_arena[xx, yy] == 1:
                        self.last_arena[xx, yy] = FREE
            ...
            crate_positions = self.rng.permutation(
                all_positions[self.last_arena == CRATE]
            )
            free_positions = self.rng.permutation(
                all_positions[self.last_arena == FREE]
            )
            coin_positions = np.concatenate([crate_positions, free_positions], 0)[
                : scenario_info["COIN_COUNT"]
            ]
            for x, y in coin_positions:
                self.last_coins.append(
                    Coin((x, y), collectable=self.last_arena[x, y] == FREE)
                )
            self.last_start_position_permutation = self.rng.permutation(start_positions)

            # Reset agents and distribute starting positions
        active_agents = []
        for agent, start_position in zip(
            self.agents, self.last_start_position_permutation
        ):
            active_agents.append(agent)
            agent.x, agent.y = start_position

        return self.last_arena.copy(), copy.deepcopy(self.last_coins), active_agents
