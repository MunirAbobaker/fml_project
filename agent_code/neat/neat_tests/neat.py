...
WEIGHT_DIFF_COEFFICIENT = 4  # How important is difference in weights for speciation
DIFFERENCE_THRESHOLD = (
    1.5  # The threshold which needs to be exceeded to be put into a new specie
)
COMPLETELY_MUTATE_WEIGHT_CHANCE = 0.01  # Chance to randomize the weight instead of slightly modifying it when mutate_weights is triggered
...
class Genome:
    ...
    def visualize(self, surface: pygame.Surface):
        ...
        input_h = math.ceil(math.sqrt(INPUT_SIZE * 2))
        input_w = input_h // 2
        hidden_h = math.ceil(math.sqrt(MAX_HIDDEN * 2))
        hidden_w = hidden_h // 2
        ...
        for x in range(output_w):
            for y in range(output_h):
                positions[x * output_h + y + INPUT_SIZE + MAX_HIDDEN] = (
        ...
        for connection in self.connectionGenes.values():
            ...
            pygame.draw.line(
                surface, color, positions[connection.from_], positions[connection.to_]
            )
    ...
    def add_node(self):
        ...
        id = INPUT_SIZE if (not _hn) else max(self.get_hidden_nodes_ids()) + 1
