...
def setup(self):
    ...
    if self.train or not os.path.isfile('my-saved-model.pt'):
        ...
        self.training_strategy_fn = EGreedyExpStrategy(init_epsilon=1.0,
                                                          min_epsilon=0.3,
                                                          decay_steps=20000,
                                                          preprocessor=preprocessor,
                                                          logger=self.logger)

        self.Q_eval = DeepQNetwork(input_dims=settings.INPUT_DIMENSIONS,
                                   n_actions=len(settings.ACTIONS), training_strategy_fn=self.training_strategy_fn,
                                   logger=self.logger, lr=0.99).float()
    else:
...
def act(self, game_state: dict) -> str:
    ...
    action = self.Q_eval.act(train=self.train, valid_actions=valid_actions,
                                         action_ideas=action_ideas, game_state=game_state)
